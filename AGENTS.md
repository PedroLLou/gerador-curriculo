...
> Arquivo padrão e neutro de instruções para agentes (Cursor, Codex e outras ferramentas).
> Tem o **mesmo conteúdo** do `CLAUDE.md` (usado pelo Claude Code). Mantenha os dois em sincronia.
> Vendor-neutral agent instructions file (Cursor, Codex and other tools). It has the **same content**
> as `CLAUDE.md` (used by Claude Code). Keep both in sync.
